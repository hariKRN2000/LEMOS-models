...
def pid_controller(protein, set_point, controller_gain, error_array, time_array, tau_I, integral_error, tau_D):
    # Proportional term
    ...
    if len(error_array) > 1:
        # Calculate the change in error over the change in time for the derivative term
        d_error = (error_array[-2] - error_array[-1]) / (time_array[-2] - time_array[-1])
    else:
        d_error = 0  # No derivative term if there's not enough data yet
    ...
    time_green = controller_gain * (p_term + i_term + d_term)

    control_interval = 8

    # Clamp the green time to a maximum of 10 minutes
    # time_green = min(max(time_green, 0), 8)
    # time_red = 8 - time_green
...
def run_pid_control(total_time, set_point, gain, tau_I, tau_D, initial_conditions, params):
    ...
    while current_time < total_time:
        ...
        time_green, time_red = pid_controller(protein_concentration, ctrl_point, gain, error_array, 
                                             time_array, tau_I, integral_error, tau_D)
