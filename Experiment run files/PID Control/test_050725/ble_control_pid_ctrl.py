...
sys.stdout = TeeLogger("terminal_output_log.txt")
...
# PID Controller Parameters (for pid_control_wells)
K_pid = 0.032        # overall gain
tau_I_pid = 1500*60  # integral time constant in seconds
tau_D_pid = 120*60      # derivative time constant in seconds
...
## Adjust the cell names to match your setup
neg_well = 'H11'
g_ctrl_wells = ['B9', 'C9', 'D9']
r_ctrl_wells = ['E9', 'F9', 'G9']
stpt2_P_wells  = []
stpt1_P_wells  = []
stpt2_PID_wells  = ['B11',  'C11', 'D11']
stpt1_PID_wells  = ['E11',  'F11',  'G11']
stpt2_PI_wells  = []
stpt1_PI_wells  = []
neg_index = ctrl_wells.index(neg_well)

# Define what control strategy in what well:
p_control_wells = stpt2_P_wells + stpt1_P_wells
pi_control_wells = stpt2_PI_wells + stpt1_PI_wells
pid_control_wells = stpt2_PID_wells + stpt1_PID_wells

# Data storage for PID control
errors = {well: [] for well in ctrl_wells}
pi_integrals = {well: 0.0 for well in pi_control_wells + pid_control_wells}
ctrl_times_stamp = []
error_df = pd.DataFrame(columns=['time'] + pi_control_wells + pid_control_wells)
...
datafile_filename = 'Datafile/test_050725.csv'
...
def run_experiment():
    ...
    try:
        ...
        print("No continue button found, will wait for 10 seconds...")
        time.sleep(10)
        continue_loc = pyautogui.center(pyautogui.locateOnScreen("search_targets/2.continue.PNG"))
        pyautogui.click(x=continue_loc.x, y=continue_loc.y)
        time.sleep(7)
    ...
    try:
        ...
        print("No load plate button found, will wait for 10 more seconds...")
        time.sleep(10)
        load_plate_loc = pyautogui.center(pyautogui.locateOnScreen("search_targets/3.loadplate.PNG"))
        pyautogui.click(x=load_plate_loc.x, y=load_plate_loc.y)
...
class FileHandler(watchdog.events.PatternMatchingEventHandler):
    ...
    def on_modified(self, event):
        ...
        global error_df, ctrl_times_stamp, errors, pi_integrals
        ...
        ctrl_times_stamp.append(curr_time)
        ...
        for i in range(len(ctrl_wells)):
            ...
            if ctrl_wells[i] in stpt2_P_wells or ctrl_wells[i] in stpt2_PI_wells or ctrl_wells[i] in stpt2_PID_wells:
                ctrl_pt = st_pt_2

            error = ctrl_pt - fl_by_od
            errors[ctrl_wells[i]].append(error)  # Save error for all wells

            if ctrl_wells[i] in pid_control_wells:
                # PID Control Wells 
                if len(ctrl_times_stamp) >= 2:
                    pi_integrals[ctrl_wells[i]] = compute_integral(ctrl_times_stamp, errors[ctrl_wells[i]])
                    derivative = compute_derivative(ctrl_times_stamp, errors[ctrl_wells[i]])
                    
                    # Apply your requested formula
                    on_time = K_pid * (error + tau_D_pid * derivative + (1 / tau_I_pid) * pi_integrals[ctrl_wells[i]])
                else:
                    on_time = K_pid * error  # Not enough data points yet

            elif ctrl_wells[i] in pi_control_wells:
                # PI Control Wells
                if len(ctrl_times_stamp) >= 2:
                    pi_integrals[ctrl_wells[i]] = compute_integral(ctrl_times_stamp, errors[ctrl_wells[i]])
        ...
        for well in pi_control_wells + pid_control_wells:
            if len(errors[well]) > 0:
                row[well] = errors[well][-1]
            else:
                row[well] = ""

        error_df = pd.concat([error_df, pd.DataFrame([row])], ignore_index=True)
        error_df.to_csv('Datafile/errors.csv', index=False)
...
def compute_derivative(times, errors):
    """
